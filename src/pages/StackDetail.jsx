...
export default function StackDetail() {
  ...
  useEffect(() => {
    Promise.all([
      base44.entities.Stack.filter({ slug }, '-created_date', 1),
      base44.entities.Tool.list('-created_date', 50),
    ]).then(([stacks, allTools]) => {
      const found = stacks[0] || null;
      setStack(found);
      setTools(allTools);
      if (found) {
        base44.entities.Stack.filter({ business_type: found.business_type }, '-created_date', 10).then(related => {
          setRelatedStacks(related.filter(s => s.slug !== found.slug));
        });
      }
      setLoading(false);
    });
  ...
  if (!stack) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <Link to="/stacks"><Button variant="outline">Back to stacks</Button></Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
      <Link to="/stacks" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-8 transition-colors">
                    ...
                    {step.tools && step.tools.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {step.tools.map(toolName => {
                          ...
                          return tool ? (
                            <Link key={toolName} to={`/tools/${tool.slug}`}>
      ...
      {/* Who / When / When Not — 3-column grid */}
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-wrap gap-2">
            {stack.alternatives.map(alt => {
              ...
              return altTool ? (
                <Link key={alt} to={`/tools/${altTool.slug}`}>
