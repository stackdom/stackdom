...
export default function StackBuilder() {
  ...
  useEffect(() => {
    base44.entities.Tool.list('-created_date', 100).then(allTools => {
      setTools(allTools);
      // Derive unique categories from live tool data for the "needs" step
  ...
  const getRecommendedTools = () => {
    ...
    return needs.map(need => {
      // Get all tools in this category, filter by budget
      const candidates = tools
        .filter(t => t.category === need)
        .filter(t => t.monthly_price == null || t.monthly_price <= maxPrice);

      // Sort: for Simple prefer cheaper, for Advanced prefer higher-priced
